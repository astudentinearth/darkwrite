...
Install packages by invoking `pnpm install` im `/app`. When the dependencies are installed, run `pnpm dev`.
...
Darkwrite uses `pnpm` for package management. `vite-plugin-electron` is used to bundle the app. Changes to source code triggers hot module reload during development. Changes to main process code automatically restarts the app.
