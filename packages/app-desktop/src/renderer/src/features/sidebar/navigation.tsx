...
export function NavigationWidget(){
    ...
    return <div className="rounded-[12px] flex flex-col gap-0.5">
        <Button onClick={()=>navigate("/")} variant={"ghost"} 
            className={
                ...
                location.pathname === "/" ? "text-foreground bg-secondary/80" : "text-foreground/60")}>
            <House size={16}></House>
        ...
        <Button onClick={()=>navigate("/settings")} variant={"ghost"} 
            className={
                cn("rounded-[8px] hover:bg-secondary/50 text-foreground/75 hover:text-foreground active:bg-secondary/25 transition-colors grid grid-cols-[24px_1fr] select-none p-1 pl-2 h-8 overflow-hidden", 
                location.pathname === "/settings" ? "text-foreground bg-secondary/80" : "text-foreground/60")}>
