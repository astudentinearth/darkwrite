...
export function StyleUI(props: { customizations: NoteCustomization, noteId: string }) {
  const {
    backgroundColor,
    coverImageSource,
    customFont,
  ...
  const {t} = useTranslation("translation", {keyPrefix: "editor.customizations"});
  ...
  return <div className="flex flex-col">
    <div className="grid gap-1 grid-cols-[1fr_1fr_1fr_1fr] grid-rows-1  [&>button]:h-fit [&>button]:flex [&>button]:flex-col [&>button]:gap-1 [&>button]:rounded-xl [&>button]:w-20">
      <Button onClick={()=>setFont(FontStyle.SANS)} variant={"ghost"} className={cn((font === FontStyle.SANS || !font) && "text-primary-text hover:text-primary-text")}>
        <span className="darkwrite-sans! text-3xl">Aa</span>
        <span>{t("sansText")}</span>
      </Button>
      <Button onClick={()=>setFont(FontStyle.SERIF)} variant={"ghost"} className={cn(font === FontStyle.SERIF && "text-primary-text hover:text-primary-text")}>
        <span className="darkwrite-serif! text-3xl">Aa</span>
        <span>{t("serifText")}</span>
      </Button>
      <Button  onClick={()=>setFont(FontStyle.MONO)}variant={"ghost"} className={cn(font === FontStyle.MONO && "text-primary-text hover:text-primary-text")}>
        <span className="darkwrite-mono! text-3xl">Aa</span>
        <span>{t("monoText")}</span>
      </Button>
      <Button onClick={()=>setFont(FontStyle.CUSTOM)} variant={"ghost"} className={cn(font === FontStyle.CUSTOM && "text-primary-text hover:text-primary-text")}>
